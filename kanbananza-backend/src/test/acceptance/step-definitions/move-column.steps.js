...
const thenBoardLooksAsFollows = (then) => {
    then("the board columns will look as follows:", async (table) => {
      ...
      for (const row of table) {
        let matchingCol = cols.find((col) => col.order === parseInt(row.columnOrder));
  ...
  const whenUserAttemptsToMoveColumn = (when) => {
    when(/^the user attempts to move column with name "(.*)" to index (\d+)$/, async (name, index) => {
        const matchingCols = (await request.get(`/board/${selectedBoard}/columns?name=${name}`)).body;
        let colId;
...
defineFeature(feature, (test) => {
    test('Move the column in the board (Normal Flow)', ({ given, when, then }) => {
    ...
    test('Move a column to the same position (Alternate Flow)', ({ given, when, then }) => {
    	givenExistsUser(given);
