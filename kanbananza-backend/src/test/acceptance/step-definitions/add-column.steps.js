...
const feature = loadFeature("src/test/acceptance/features/ID007_Add-Column.feature");
...
defineFeature(feature, (test) => {
  ...
  test('Successfully add a column with a valid name to an empty board (Normal Flow)', ({
    ...
    when(
      /^the user attempts to create a column with name "(.*)"$/, (name) => {

      }
    );

    then(
     /^the board contains a column with name "(.*)"$/, (name) => {

     }
    );

    then(
      "the board contains one column",
      () => {} 
    );
  });
  ...
  test('Successfully add a column with a valid name to a board with existing columns (Alternate Flow)', ({
    ...
    then(
      "^the board contains 4 columns",
      () => {}
    );

    then("the columns in the board shall have the following names and order:", () => {});
  });
